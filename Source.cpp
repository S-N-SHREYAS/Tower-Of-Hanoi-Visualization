...
#define POLEBASEHEIGHT 0.1f
#define POLEBASERAD 0.025f
#define SLICES 32
#define INNERSLICES 16
#define LOOPS 1
#define FPS 64   /* more looks nicer, uses more cpu power */
#define FEM 1000.0/FPS
#define FSEM 0.001f   /* speed (bigger value => faster)*/
...
void drawDisk(GLUquadricObj** quadric, const GLfloat outer, const GLfloat inner)
{
	...
	gluCylinder(*quadric, outer, outer, POLEBASEHEIGHT, SLICES, LOOPS);
	gluQuadricOrientation(*quadric, GLU_INSIDE);
	if (inner > 0)
		gluCylinder(*quadric, inner, inner, POLEBASEHEIGHT, INNERSLICES, LOOPS);
	gluDisk(*quadric, inner, outer, SLICES, LOOPS);
	gluQuadricOrientation(*quadric, GLU_OUTSIDE);
	glTranslatef(0.0, 0.0, POLEBASEHEIGHT);
	gluDisk(*quadric, inner, outer, SLICES, LOOPS);
	gluQuadricOrientation(*quadric, GLU_OUTSIDE);
	glPopMatrix();
}
...
void drawPin(GLUquadricObj** quadric, const GLfloat radius, const GLfloat height)
{
	...
	glRotatef(-90.0, 1.0, 0.0, 0.0);
	gluCylinder(*quadric, radius, radius, POLEBASEHEIGHT / 2, SLICES, LOOPS);
	gluQuadricOrientation(*quadric, GLU_INSIDE);
	gluDisk(*quadric, 0.0, radius, SLICES, LOOPS);
	gluQuadricOrientation(*quadric, GLU_OUTSIDE);
	glTranslatef(0.0, 0.0, POLEBASEHEIGHT / 2);
	gluDisk(*quadric, 0.0, radius, SLICES, LOOPS);
	gluCylinder(*quadric, POLEBASERAD, POLEBASERAD, height, INNERSLICES, LOOPS);
	glTranslatef(0.0, 0.0, height);
	gluDisk(*quadric, 0.0, POLEBASERAD, INNERSLICES, LOOPS);
	glPopMatrix();
}
...
void hanoiinit(void)
{
	...
	config.pinheight = disks * POLEBASEHEIGHT + 0.2f;
...
void Display(void)
{
	...
	char* p7 = (char*)s7;
	drawBitmapString(-32.0, 22.0, -60.0, GLUT_BITMAP_9_BY_15, p7);

	const char* s8 = "f : fullscreen";
	char* p8 = (char*)s8;
	drawBitmapString(-32.0, 20.0, -60.0, GLUT_BITMAP_9_BY_15, p8);

	const char* s9 = "q/Esc : quit";
	char* p9 = (char*)s9;
	drawBitmapString(-32.0, 18.0, -60.0, GLUT_BITMAP_9_BY_15, p9);

	const char* s13 = "Press 1 - 9 to select the number of disks";
	char* p13 = (char*)s13;
	drawBitmapString(-32.0, 16.0, -60.0, GLUT_BITMAP_9_BY_15, p13);
	...
	glTranslatef(-config.gap, POLEBASEHEIGHT / 2, 0.0);
	glPushMatrix();
	...
	{ /* fill pins with disks */
		...
		if ((cur = pin[i].bottom) != NULL)
		{
			do {
				setColor(cur->color);
				drawDisk(&quadric, cur->radius, POLEBASERAD);
				glTranslatef(0.0, POLEBASEHEIGHT, 0.0);
				pinheight[i] += POLEBASEHEIGHT;
				cur = cur->next;
			} while (cur != NULL);
		}
		glPopMatrix();
	...
	if (curaction != NULL && curaction->fromstack != -1 && curdisk != NULL) {
		...
		drawDisk(&quadric, curdisk->radius, POLEBASERAD);
	}
